    ...
    RadioGroup.OnCheckedChangeListener
{
    ...
    fun configure(standalone: Boolean, gameName: String?) {
        ...
        if (hasKnowns) {
            ...
            mNamesSpinner = findViewById<View>(R.id.names) as Spinner
            mNamesSpinner!!.adapter = adapter
            mNamesSpinner!!.onItemSelectedListener = this
        ...
        for (resID in toHide) {
            findViewById<View>(resID).visibility = GONE
        }

        val et = findViewById<View>(R.id.name_edit) as EditWClear
        et.setText(gameName)

        val group = findViewById<View>(R.id.group) as RadioGroup
        group.setOnCheckedChangeListener(this)

        val key = if (standalone) KP_PREVSOLO_KEY else KP_PREVNET_KEY
        val lastSet = DBUtils.getIntFor(context, key, 0)
        if (lastSet != 0) {
            ...
            val view = findViewById<View>(lastSet)
            if (null != view && view is RadioButton) {
                group.check(lastSet)
            }
        }
    }
    ...
    private fun onRadioChanged() {
        if (null != mNamesSpinner) {
            mNamesSpinner!!.visibility = if (mCurRadio == R.id.radio_known
            ) VISIBLE else GONE
        }
        ...
        var msg: String? = null
        when (mCurRadio) {
            R.id.radio_known -> msg = LocUtils.getString(context, R.string.newgame_invite_fmt, mCurKnown)
