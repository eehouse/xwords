...
showThumb( void* closure, int XP_UNUSED(key) )
{
    ...
    if ( !!gr ) {
        ...
        if ( !!stream ) {
            ...
            ca_informf( aGlobals, "Here's your thumbnail for %X: \n%s",
                        gr, str );
            free( str );
            strm_destroy( stream );
        }
    } else {
        ca_inform( aGlobals, "No thumbnails for groups!" );
    }
    return true;
}
...
toggleGroupExpanded( void* closure, int XP_UNUSED(key) )
{
    ...
    if ( !!grp ) {
        ...
        ca_inform( aGlobals, "No expanding games!" );
    }
    return true;
}
